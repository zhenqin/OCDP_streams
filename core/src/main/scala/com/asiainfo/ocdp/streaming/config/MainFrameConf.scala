...
import com.asiainfo.ocdp.streaming.constant.TableInfoConstant
import com.asiainfo.ocdp.streaming.tools.Json4sUtils
...
object MainFrameConf {
  ...
  val beid2BeconfMap = mutable.Map[String, BusinessEventConf]()
...
//      val propsMap = Json4sUtils.jsonStr2ArrTuple2(propsJsonStr, Array("uks", "ukSeparator"))
      ...
      //设置使用自定义增强类
      val className = dataInterfaceConf.get("class_name", "com.asiainfo.ocdp.streaming.datasource.DataInterface")
...
//      diid2EventId2ParentEventId.get(diid).get.put(erid, peventid)
  ...
  def initBusinessEventConf() {
    ...
    val businessEvents = mutable.ArrayBuffer[BusinessEventConf]()

    dsdata.foreach(x => {
      //每行记录对应一个业务配置
      val beConf: BusinessEventConf = new BusinessEventConf()
